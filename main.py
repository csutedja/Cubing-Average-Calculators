...
if question == str(1):
  sum_of_times = 0
  for i in range(0,3):
    time_input = float(input("Please input a time, seperating each one with ENTER key "))
    time_list.append(time_input)
  sum_of_times = sum([float(x) for x in time_list])
  mean_of_three = sum_of_times / 3
  print("Your mean of 3 is " + str(round(mean_of_three, 2)) + " seconds.")
 ...
 ao5 = three_solves/3
 print("Your average of 5 is " + str(round(ao5, 2)) + " seconds.")
  ...
  print("Your average of 12 is " + str(round(ao12, 2)) + " seconds.")
